...
实施清单（M3 完成要求）
- 科技树：收益/时间比排序；依赖闭包与拓扑排序；研究速率与进度计算；解锁集更新。
- 订单规划：目标节拍→方案生成与可行性判定；最大流/最短路径与设备等级/带速约束。
- 瓶颈分析与建议：Slack/利用率/敏感度分析；生成升级/分流/优先级/电力/燃料建议并可应用对比。
- 成本与良率：持有/缺货/能耗成本模型与优化目标；良率/报废/返修流配置与统计。
- 周期与交期：CT/LT 计算、`promiseDate/dueRisk` 评估与加急建议展示。
- 页面：`/factory/research`、`/factory/planning`、`/factory/orders` 视图与交互。

补充验收
- 方案生成在中等规模 `< 2s`；瓶颈与建议方向性正确，应用后吞吐提升或权衡明确。
- 成本面板与模式切换生效；有效产能与仿真吞吐一致；良率变化反映在产出与报废统计。
- 周期与交期计算与观测一致（误差在阈内）；高风险场景下加急建议明显改善或给出权衡。

验收用例列表（M3）
- 创建包含三层工艺链的订单，生成方案 `<2s`，并给出瓶颈与建议；应用建议后吞吐提升≥15%。
- 切换拉/推模式：拉式满足交期目标，推式持有成本下降且服务水平≥目标；成本面板数值联动。
- 设置良率 0.9：产出与报废统计一致；有效产能=名义产能×0.9 与吞吐吻合。
- 计算 CT/LT 并与仿真观测对比，误差在设定阈值内；高风险订单触发加急建议。

现状与代码映射（M3）
- 领域模型
  - Items/Recipes/Machines：`src/libs/factory/Factory.Domain/Entities/Item.cs:1-3`、`src/libs/factory/Factory.Domain/Entities/Recipe.cs:1-8`、`src/libs/factory/Factory.Domain/Entities/Machine.cs:1-9`
- 应用端点
  - 订单/规划/研究相关端点：暂无（待实现）。
- 仿真与规划
  - 现有仿真步进：`src/libs/factory/Factory.Simulation/Engine/SimulationEngine.cs:25-52`；规划器与瓶颈分析：暂无（待实现）。
