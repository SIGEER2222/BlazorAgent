...
现状与代码映射（M5）
- 报表页面：`/reports` → `src/apps/Factory.Web/Components/Pages/FactoryReports.razor:1-35`
- 诊断与导出相关端点/页面：暂无（待实现）。
...
迭代任务清单（M5）
- 指标采集与采样节流：TPS/P95 延迟/CPU/Memory/GC/DroppedTicks，页面与 API 对齐。
- 故障注入与稳态识别：丢包/限电/延迟/燃料中断，恢复阈值与监控面板。
- 并行与数据结构优化：池化、锁竞争规避、采样与 UI 刷新节流。
- 导出与数据血缘：CSV/Excel 模板与字段/单位、元数据（版本/采样频率/窗口大小）。
- 扩展注册沙箱：资源配额、异常隔离与熔断，JSON Schema 校验与签名（可选）。

验收用例列表（M5）
- `N=10k` 节点场景下：TPS≥30、P95≤50ms、Memory≤2GB；采样节流不影响核心吞吐。
- 故障注入：限电 50%→吞吐约减半；丢包与延迟→DroppedTicks/Latency 指标上升；`RecoveryTime≤10s`。
- 导出 CSV/Excel 含血缘元数据，字段单位正确；随机抽样复核与页面一致。
- 扩展注册：不合规载荷被拒并返回错误详情；合规载荷注册成功并隔离运行。
